...
async function perBatchOf50Streams (logStreams) {
  ...
  for (let i = 0; i < logStreams.length; i++) {
    ...
    if (logStreamName.indexOf('_') === -1) { batchResults.invalidLogStreamName.push(logStreamName) }

    const params = { logGroupName, logStreamName }
    const logEventsForCount = await getLogEvents(params)

    if (!logEventsForCount.events.length) { batchResults.noEventsForLogStream.push(logStreamName) }
...
module.exports = async (awsLogFuncResult) => {
  ...
  Object.keys(batchResults).forEach(key => {
    ...
    if (totalForAllStreams.hasOwnProperty(key)) {
      if (typeof value === 'number') { totalForAllStreams[key] += value } else { totalForAllStreams[key] = totalForAllStreams[key].concat(value) }
    } else { totalForAllStreams[key] = value }
  })
